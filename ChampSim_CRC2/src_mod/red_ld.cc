...
#define RED_SETS_BITS 10						//number of bits used to index the ART structure
...
uint32_t GetVictimInSet (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
	...
	if (type == LOAD || type == RFO || type == PREFETCH) {
		...
		if (!present) {
			...
			// bypass when address not in ART and reuse in PCRT is low or intermediate //ykhodke:: this is where the threshold value is set for bypassing
