...
{
    uint16_t pc0 = pc & 0x3ff;
    uint8_t bit32 = (pc & 0x1000000) >> 24;
    uint8_t bit40 = (pc & 0x100000000) >> 32;
    uint16_t hash = pc0 + (bit40 <<12) + (bit32<<11) ;    
    return hash;
}

uint64_t hash37(uint64_t tag_addr)      //hashing function for mrmory address to 37 bits
{
    uint64_t top8 = (tag_addr<<(6+LLC_SETS_LOG2+ 7 )) >> 56;
    uint64_t hash = (top8<<20) + (tag_addr&0xfffff);
    return hash;
}
