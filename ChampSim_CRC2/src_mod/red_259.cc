...
#define RED_SETS_BITS 9						//number of bits used to index the ART structure
#define RED_WAYS 16								//associativity of each set in ART set
#define RED_TAG_SIZE_BITS 11			//the size of tag bits used for the addresses.
#define RED_SECTOR_SIZE_BITS 2
#define RED_PC_FRACTION 4					//number of PCs that can correlate to one art entry.
#define PCs_BITS 8								//number of bits used to index PC.
...
void remember(uint32_t cpu, uint64_t PCnow, uint64_t block)
{
	...
	PCnow_entry = ((PCnow >> 2) & (0x1DF)) & (PCs-1); //ykhodke:: sliding window method to generate PCnow_entry
...
uint32_t GetVictimInSet (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
	...
	//ykhodke:: this is where the threshold value is set for bypassing
